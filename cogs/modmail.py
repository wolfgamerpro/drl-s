...
class Utility(commands.Cog):
    ...
    @staticmethod
    def _verify_user_or_role(Usuario_O_Rol):
        if isinstance(Usuario_O_Rol, discord.Role):
            if Usuario_O_Rol.is_default():
                return -1
        elif Usuario_O_Rol in {"everyone", "all"}:
            return -1
        if hasattr(Usuario_O_Rol, "id"):
            return Usuario_O_Rol.id
        raise commands.BadArgument(f'Usuario o rol "{Usuario_O_Rol}" no encontrado')
    ...
    async def permissions_add(
        ...
        name: str,
        *,
        Usuario_O_Rol: Union[discord.Role, utils.User, str],
    ):
        """
        ...
        value = self._verify_user_or_role(Usuario_O_Rol)
        if type_ == "command":
            ...
            if level > PermissionLevel.REGULAR:
                ...
                elif isinstance(Usuario_O_Rol, discord.Role):
                    key = Usuario_O_Rol
                else:
    ...
    async def permissions_remove(
        ...
        name: str,
        *,
        Usuario_O_Rol: Union[discord.Role, utils.User, str] = None,
    ):
        """
        ...
        if type_ not in {"command", "level", "override"} or (
            type_ != "override" and Usuario_O_Rol is None
        ):
            return await ctx.send_help(ctx.command)

        if type_ == "override":
            extension = ctx.kwargs["Usuario_O_Rol"]
        ...
        if type_ == "level":
            if level > PermissionLevel.REGULAR:
                if value == -1:
                    ...
                    logger.info("Denying %s access to Modmail category.", Usuario_O_Rol.name)
                    await self.bot.main_category.set_permissions(Usuario_O_Rol, overwrite=None)
                else:
    ...
    async def permissions_get(
        self, ctx, Usuario_O_Rol: Union[discord.Role, utils.User, str], *, name: str = None
    ):
        """
        ...
        if name is None and Usuario_O_Rol not in {"command", "level", "override"}:
            value = self._verify_user_or_role(Usuario_O_Rol)
            ...
            mention = getattr(Usuario_O_Rol, "name", getattr(Usuario_O_Rol, "id", Usuario_O_Rol))
            desc_cmd = (
            ...
            Usuario_O_Rol = (Usuario_O_Rol or "").lower()
            if Usuario_O_Rol == "override":
                if name is None:
                    done = set()
            ...
            if Usuario_O_Rol not in {"command", "level"}:
                return await ctx.send_help(ctx.command)
            embeds = []
            if name is not None:
                name = name.strip('"')
                command = level = None
                if Usuario_O_Rol == "command":
                    name = name.lower()
                    command = self.bot.get_command(name)
                    check = command is not None
                else:
                ...
                if not check:
                    embed = discord.Embed(
                        title="Error",
                        color=self.bot.error_color,
                        description=f"La referencia {Usuario_O_Rol} no existe: `{name}`.",
                    )
                    return await ctx.send(embed=embed)
